...
class DoubleWallsLangevin():
    ...
    def sample(self):
        ...
        # z = np.linspace(-self.H, self.H, int(1e4))
        z = np.concatenate((np.linspace(-self.H, -self.H+5*self.lB, int(1e4)), np.linspace(-self.H+5*self.lB, self.H, 1000))) # Increase precision close the zone of interest!
